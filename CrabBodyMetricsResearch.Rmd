...
The resulting graph, discriminated by the third value of the species themselves, shows four distinct lines. the dashed representing the comparison against Body depth and the straight line against Frontal Lobe Size. With these values it can be concluded that the Blue Leptograspus are overall wider than the Orange Leptograspus, and thus this data can be used for machine learning purposes and creating the algorithm that when fed the proper measurements, can determine whether the given crab's data belongs to an individual belonging to the Blue Leptograspus or one belonging to the Orange Leptograspus. 
...
```{r, fig.cap="A table showing the overall accuracy of the classification method along with the significant after 100 runs of each individual method."}
