...
pie(c(length(CrabData$sp == "Orange Leptograspus"), length(CrabData$sp == "Blue Leptograspus")), c("Orange Leptograspus", "Blue Leptograspus"), main="Distribution of data", col = c("Orange", "Blue"))
...
```{r, echo=FALSE, fig.width=6.5,fig.height=3}
# CrabMeans <- kmeans(na.omit(CrabData[c(4,7)]),2)
# CrabMeans
# library(cluster)

# PCAplot(CrabData, CrabMeans$cluster, color=TRUE, shade=TRUE)
# dd <- dist(scale(USArrests), method = "euclidean")
# hc <- hclust(dd, method = "ward.D2")

# CrabTibble <- tibble(CrabData[,c(1,4,7)])
# 
# CrabTibble_long <- pivot_longer(data = CrabTibble,
#                                 cols = c(A.B),
#                                 names_to = "property",
#                                 values_to = "value")
...
print(CrabData.pca)
summary(CrabData.pca)
plot(CrabData.pca, type="l")
ggbiplot(CrabData.pca, obs.scale = 1, var.scale = 1, 
              groups = CrabData[4:8], ellipse = FALSE, 
              circle = TRUE) + 
